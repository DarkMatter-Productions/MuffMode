...
| Ghost runtime test seam | Enforced in CI | `windows-2025-vs2026` | MSVC `v143` via MSBuild | `Release|x64` | Strictly compiles the runtime-soak-only reconnect identity seam with `MMGhostRuntimeTesting=true`, preventing its engine-facing lifecycle hooks from silently rotting outside the host-test target. |
