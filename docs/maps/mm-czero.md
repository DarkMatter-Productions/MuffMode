# Cold Zero

[Map Guide](index.md) | [Gameplay Reference](../gameplay-reference.md)

| Field | Details |
| --- | --- |
| Filename | `mm-czero` |
| Development status | Final release |
| Original map | ven_dm2 / Cold Zero |
| Original author | Kev "Ven" Pritchard |
| Original release date | 22 April 2001 |
| Recommended players | 3-8 |
| Good fits | FFA, 2v2, TDM, Instagib |
| Original readme | Not located |
| Original BSP | [ven_dm2.bsp](original-bsps/ven_dm2.bsp) |
| Remaster source map | [mm-czero.map](source-maps/mm-czero.map) |

## Why Play It

A lean contest-map layout with open reads, strong rail lines, and enough health to keep a public server from stalling.

## Where It Came From

Cold Zero came from Ven and is tied to PlanetQuake's Not Dead Yet! 600-brush contest coverage. That constraint gives the layout a pleasantly spare quality: readable, quick, and hard to hide in.

## How It Plays

Best for FFA and light team play. It has nine deathmatch starts, a full weapon spread, and a Mega Health, so it can absorb more players than the pure duel maps.

## Item Register

Counts are taken from the final Muff Mode BSP entity data.

| Category | Register |
| --- | --- |
| Spawns | Deathmatch spawn x9, Intermission camera x1, Start point x1 |
| Weapons | Chaingun x1, Grenade Launcher x1, HyperBlaster x1, Machinegun x1, Railgun x1, Rocket Launcher x1, Shotgun x1, Super Shotgun x1 |
| Ammo | Bullets x6, Bullets (large) x1, Cells x3, Grenades x4, Rockets x4, Shells x4, Slugs x2 |
| Utility | Adrenaline x1, Combat Armor x2, Jacket Armor x1, Armor Shard x12, Health x7, Large Health x5, Mega Health x1, Small Health x6, Ammo Pack x1 |

## Preserved Files

| File type | Link |
| --- | --- |
| Remaster source map | [mm-czero.map](source-maps/mm-czero.map) |
| Original readme | Not located |
| Original BSP | [ven_dm2.bsp](original-bsps/ven_dm2.bsp) |

## Sources

- Final Muff Mode BSP/source data: [source-maps/mm-czero.map](source-maps/mm-czero.map).
- Preserved original BSP: [original-bsps/ven_dm2.bsp](original-bsps/ven_dm2.bsp).
- PlanetQuake Level of the Week: [https://planetquake.gamespy.com/View38ff.html?id=177&view=LOTW.Detail](https://planetquake.gamespy.com/View38ff.html?id=177&view=LOTW.Detail).
