# Reclamation

[Map Guide](index.md) | [Gameplay Reference](../gameplay-reference.md)

| Field | Details |
| --- | --- |
| Filename | `mm-reclam` |
| Development status | Final release |
| Original map | kamq2dm3 / q2rdm7 |
| Original author | Corey "Kamarov" Peters |
| Original release date | 4 March 1999 |
| Recommended players | 2-4 |
| Good fits | Duel, small FFA |
| Original readme | Not located |
| Original BSP | [kamq2dm3.bsp](original-bsps/kamq2dm3.bsp) |
| Remaster source map | [mm-reclam.map](source-maps/mm-reclam.map) |

## Why Play It

A compact Kamarov map with a disciplined weapon set, modest health, and no oversized powerup safety net.

## Where It Came From

Reclamation is preserved as kamq2dm3 and is also associated with q2rdm7. The nearby kamq2dm4 readme mentions it as Kamarov's earlier Reclamation, but that readme belongs to Ubiquity and is not treated as this map's original readme.

## How It Plays

A strong small-session map when the server wants quick fights without Quad or Mega Health. Four deathmatch starts keep it intimate.

## Item Register

Counts are taken from the final Muff Mode BSP entity data.

| Category | Register |
| --- | --- |
| Spawns | Deathmatch spawn x4, Intermission camera x1, Start point x1 |
| Weapons | Chaingun x1, Grenade Launcher x1, HyperBlaster x1, Railgun x1, Rocket Launcher x1, Super Shotgun x1 |
| Ammo | Bullets x4, Cells x2, Grenades x1, Rockets x3, Shells x1, Slugs x1 |
| Utility | Combat Armor x1, Jacket Armor x1, Armor Shard x7, Health x4, Large Health x1 |

## Preserved Files

| File type | Link |
| --- | --- |
| Remaster source map | [mm-reclam.map](source-maps/mm-reclam.map) |
| Original readme | Not located |
| Original BSP | [kamq2dm3.bsp](original-bsps/kamq2dm3.bsp) |

## Sources

- Final Muff Mode BSP/source data: [source-maps/mm-reclam.map](source-maps/mm-reclam.map).
- Preserved original BSP: [original-bsps/kamq2dm3.bsp](original-bsps/kamq2dm3.bsp).
- DondeQ2 map archive: [https://dondeq2.com/2017/10/24/webman-twists-map-collection/](https://dondeq2.com/2017/10/24/webman-twists-map-collection/).
