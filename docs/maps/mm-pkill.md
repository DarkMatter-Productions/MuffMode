# Painkiller

[Map Guide](index.md) | [Gameplay Reference](../gameplay-reference.md)

| Field | Details |
| --- | --- |
| Filename | `mm-pkill` |
| Development status | Final release |
| Original map | ztn2dm1 / painklr2 |
| Original author | Sten "ztn" Uusvali |
| Original release date | 22 January 1998 |
| Recommended players | 2-4 |
| Good fits | Duel, small FFA, Clan Arena |
| Original readme | [ztn2dm1-readme.txt](original-readmes/ztn2dm1-readme.txt) |
| Original BSP | [ztn2dm1.bsp](original-bsps/ztn2dm1.bsp) |
| Remaster source map | [mm-pkill.map](source-maps/mm-pkill.map) |

## Why Play It

A tight ztn duel map with five starts, clean weapon access, and just enough item economy to punish lazy movement.

## Where It Came From

Painkiller is one of ztn's earliest Quake II duel maps and part of the same family that shaped a lot of competitive Q2 rotation thinking. Its readme names 2-4 players as the intended load.

## How It Plays

Best as a duel map. The register includes Double Damage / Haste, so hosts should decide whether that belongs in their ruleset before putting it into a serious pool.

## Item Register

Counts are taken from the final Muff Mode BSP entity data.

| Category | Register |
| --- | --- |
| Spawns | Deathmatch spawn x5, Intermission camera x2, Start point x1 |
| Weapons | Chaingun x1, Grenade Launcher x1, Machinegun x1, Railgun x1, Rocket Launcher x1, Super Shotgun x2 |
| Ammo | Bullets x3, Grenades x2, Rockets x2, Shells x2, Slugs x1 |
| Utility | Adrenaline x1, Body Armor x1, Combat Armor x2, Jacket Armor x1, Armor Shard x4, Double Damage / Haste x1, Health x3, Large Health x2, Small Health x7 |

## Preserved Files

| File type | Link |
| --- | --- |
| Remaster source map | [mm-pkill.map](source-maps/mm-pkill.map) |
| Original readme | [ztn2dm1-readme.txt](original-readmes/ztn2dm1-readme.txt) |
| Original BSP | [ztn2dm1.bsp](original-bsps/ztn2dm1.bsp) |

## Sources

- Final Muff Mode BSP/source data: [source-maps/mm-pkill.map](source-maps/mm-pkill.map).
- Preserved original readme: [original-readmes/ztn2dm1-readme.txt](original-readmes/ztn2dm1-readme.txt).
- Preserved original BSP: [original-bsps/ztn2dm1.bsp](original-bsps/ztn2dm1.bsp).
- Quake II Netpack I: Extremities: [https://archive.org/details/QuakeIINetpackIExtremitiesUSAEurope](https://archive.org/details/QuakeIINetpackIExtremitiesUSAEurope).
