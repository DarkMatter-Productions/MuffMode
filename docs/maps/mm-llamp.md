# Lava Lamp

[Map Guide](index.md) | [Gameplay Reference](../gameplay-reference.md)

| Field | Details |
| --- | --- |
| Filename | `mm-llamp` |
| Development status | Final release |
| Original map | 4u2map12a / Lava Lamp |
| Original author | [4u2]Squirrel |
| Original release date | 2003 (exact day not located) |
| Recommended players | 4-9 |
| Good fits | FFA, TDM, party server |
| Original readme | Not located |
| Original BSP | [4u2map12a.bsp](original-bsps/4u2map12a.bsp) |
| Remaster source map | [mm-llamp.map](source-maps/mm-llamp.map) |

## Why Play It

A big, energetic public-server map with rockets everywhere, nine starts, and enough lava-lit drama to justify the name.

## Where It Came From

The public listing for 4u2map12a identifies Lava Lamp as a Quake II map by [4u2]Squirrel. The local candidate readme did not match this map, so the guide preserves the BSP but treats the readme as not located.

## How It Plays

Use it when the server population climbs. It has a BFG, two rocket launchers, lots of rockets and grenades, and a Mega Health, making it better for spectacle than for quiet duel discipline.

## Item Register

Counts are taken from the final Muff Mode BSP entity data.

| Category | Register |
| --- | --- |
| Spawns | Deathmatch spawn x9, Intermission camera x1, Start point x1 |
| Weapons | BFG10K x1, Chaingun x1, Grenade Launcher x1, HyperBlaster x1, Machinegun x1, Railgun x1, Rocket Launcher x2, Shotgun x1, Super Shotgun x1 |
| Ammo | Bullets x4, Cells x3, Grenades x8, Rockets x9, Shells x4, Slugs x2 |
| Utility | Combat Armor x1, Jacket Armor x1, Armor Shard x10, Health x6, Mega Health x1, Small Health x10 |

## Preserved Files

| File type | Link |
| --- | --- |
| Remaster source map | [mm-llamp.map](source-maps/mm-llamp.map) |
| Original readme | Not located |
| Original BSP | [4u2map12a.bsp](original-bsps/4u2map12a.bsp) |

## Sources

- Final Muff Mode BSP/source data: [source-maps/mm-llamp.map](source-maps/mm-llamp.map).
- Preserved original BSP: [original-bsps/4u2map12a.bsp](original-bsps/4u2map12a.bsp).
- quake2.com.ru listing: [https://quake2.com.ru/files/maps/5/](https://quake2.com.ru/files/maps/5/).
