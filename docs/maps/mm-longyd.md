# The Longest Yard

[Map Guide](index.md) | [Gameplay Reference](../gameplay-reference.md)

| Field | Details |
| --- | --- |
| Filename | `mm-longyd` |
| Development status | Final release |
| Original map | q3dm17 / The Longest Yard |
| Original author | id Software |
| Original release date | 2 December 1999 |
| Recommended players | 4-16 |
| Good fits | FFA, Instagib, Clan Arena, jump-pad chaos |
| Original readme | Not located |
| Original BSP | Not preserved locally |
| Remaster source map | [mm-longyd.map](source-maps/mm-longyd.map) |

## Why Play It

A Quake III Arena landmark recast for Quake II: void space, long sightlines, jump-pad routes, and pure rail/rocket spectacle.

## Where It Came From

The Longest Yard is Quake III Arena's famous q3dm17, arriving with id Software's arena shooter in December 1999. This port makes the map a guest star in a Quake II ruleset.

## How It Plays

Best for Instagib, casual FFA, and novelty Clan Arena. It has sixteen starts and very sparse weapon variety, so it is about aim, movement, and not falling into space.

## Item Register

Counts are taken from the final Muff Mode BSP entity data.

| Category | Register |
| --- | --- |
| Spawns | Deathmatch spawn x16, Intermission camera x2 |
| Weapons | Railgun x1, Rocket Launcher x2, Super Shotgun x2 |
| Ammo | Bullets (large) x4, Grenades x1, Rockets x2, Shells (large) x2, Slugs x2 |
| Utility | Body Armor x1, Combat Armor x1, Armor Shard x12, Health x2, Large Health x7, Mega Health x1, Quad Damage x1 |

## Preserved Files

| File type | Link |
| --- | --- |
| Remaster source map | [mm-longyd.map](source-maps/mm-longyd.map) |
| Original readme | Not located |
| Original BSP | Not preserved locally |

## Sources

- Final Muff Mode BSP/source data: [source-maps/mm-longyd.map](source-maps/mm-longyd.map).
- Quake III Arena overview: [https://quake.fandom.com/wiki/Quake_III_Arena](https://quake.fandom.com/wiki/Quake_III_Arena).
