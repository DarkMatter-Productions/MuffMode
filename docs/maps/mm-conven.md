# Conventional

[Map Guide](index.md) | [Gameplay Reference](../gameplay-reference.md)

| Field | Details |
| --- | --- |
| Filename | `mm-conven` |
| Development status | Final release |
| Original map | grom_dm3 / Conventional |
| Original author | Robert "Grom" McLachlan |
| Original release date | 4 November 1999 |
| Recommended players | 4-8 |
| Good fits | FFA, 2v2, TDM, Quad Hog |
| Original readme | Not located |
| Original BSP | [grom_dm3.bsp](original-bsps/grom_dm3.bsp) |
| Remaster source map | [mm-conven.map](source-maps/mm-conven.map) |

## Why Play It

A broad, readable arena with enough weapon spread and a Quad to turn public FFA into a real chase.

## Where It Came From

Grom's Conventional is one of those late-classic Quake II community maps that feels built for server rotation: clear rooms, strong routes, and a name that undersells how lively it gets once the Quad comes up.

## How It Plays

Good for busy public play and light team games. The map has eight deathmatch starts, several major armors, and a Quad, so it benefits from players who will move rather than camp one room.

## Item Register

Counts are taken from the final Muff Mode BSP entity data.

| Category | Register |
| --- | --- |
| Spawns | Deathmatch spawn x8, Intermission camera x1, Start point x1 |
| Weapons | Chaingun x1, Grenade Launcher x1, HyperBlaster x1, Machinegun x2, Railgun x1, Rocket Launcher x2, Shotgun x1, Super Shotgun x1 |
| Ammo | Bullets x6, Cells x2, Grenades x7, Rockets x4, Shells x3, Slugs x2 |
| Utility | Adrenaline x1, Body Armor x1, Combat Armor x2, Jacket Armor x1, Armor Shard x13, Health x2, Large Health x5, Mega Health x1, Small Health x4, Quad Damage x1 |

## Preserved Files

| File type | Link |
| --- | --- |
| Remaster source map | [mm-conven.map](source-maps/mm-conven.map) |
| Original readme | Not located |
| Original BSP | [grom_dm3.bsp](original-bsps/grom_dm3.bsp) |

## Sources

- Final Muff Mode BSP/source data: [source-maps/mm-conven.map](source-maps/mm-conven.map).
- Preserved original BSP: [original-bsps/grom_dm3.bsp](original-bsps/grom_dm3.bsp).
- DondeQ2 map archive: [https://dondeq2.com/2017/10/24/webman-twists-map-collection/](https://dondeq2.com/2017/10/24/webman-twists-map-collection/).
