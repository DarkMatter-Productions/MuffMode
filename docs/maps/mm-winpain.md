# Window Pain

[Map Guide](index.md) | [Gameplay Reference](../gameplay-reference.md)

| Field | Details |
| --- | --- |
| Filename | `mm-winpain` |
| Development status | Final release |
| Original map | ra3map2c / Window Pain |
| Original author | Adam "Senn" Bellefeuil |
| Original release date | 24 July 2000 |
| Recommended players | 2-12 |
| Good fits | Clan Arena, Instagib, FFA warmups |
| Original readme | Not located |
| Original BSP | Not preserved locally |
| Remaster source map | [mm-winpain.map](source-maps/mm-winpain.map) |

## Why Play It

A Rocket Arena import with vertical exchanges, Plasma Beam pressure, and a simple item set that keeps the pace fast.

## Where It Came From

Window Pain is associated with Senn's ra3map2 set, a Rocket Arena 3 map pack remembered for focused arena combat. The Muff Mode version keeps that practice-arena spirit.

## How It Plays

Best for CA, Instagib, or quick FFA warmups. Twelve starts make it roomy enough for a crowd, but the weapon set is narrow and aim-heavy.

## Item Register

Counts are taken from the final Muff Mode BSP entity data.

| Category | Register |
| --- | --- |
| Spawns | Deathmatch spawn x12, Intermission camera x1 |
| Weapons | Chaingun x1, Plasma Beam x1, Railgun x1, Rocket Launcher x1, Super Shotgun x1 |
| Ammo | Bullets x2, Cells x1, Cells (small) x1, Rockets x1, Rockets (small) x1, Shells x2, Slugs (small) x1 |
| Utility | Body Armor x1, Combat Armor x1, Health x2, Large Health x2, Mega Health x1 |

## Preserved Files

| File type | Link |
| --- | --- |
| Remaster source map | [mm-winpain.map](source-maps/mm-winpain.map) |
| Original readme | Not located |
| Original BSP | Not preserved locally |

## Sources

- Final Muff Mode BSP/source data: [source-maps/mm-winpain.map](source-maps/mm-winpain.map).
- Rocket Arena 3 Standard Maps listing: [https://steamcommunity.com/sharedfiles/filedetails/?id=1804631841](https://steamcommunity.com/sharedfiles/filedetails/?id=1804631841).
- Rocket Arena 3 interview archive: [https://dondeq2.com/2018/10/03/kablooie-rocket-arena-3-interviews-with-senn-and-g1zm0/](https://dondeq2.com/2018/10/03/kablooie-rocket-arena-3-interviews-with-senn-and-g1zm0/).
