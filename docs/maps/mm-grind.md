# Grind

[Map Guide](index.md) | [Gameplay Reference](../gameplay-reference.md)

| Field | Details |
| --- | --- |
| Filename | `mm-grind` |
| Development status | Final release |
| Original map | grind / q2duel2 |
| Original author | Dennis "headshot" Kaltwasser |
| Original release date | 17 March 1998 |
| Recommended players | 2-8 |
| Good fits | Duel, 2v2, FFA |
| Original readme | [grind-readme.txt](original-readmes/grind-readme.txt) |
| Original BSP | [grind.bsp](original-bsps/grind.bsp) |
| Remaster source map | [mm-grind.map](source-maps/mm-grind.map) |

## Why Play It

A crisp early Q2DM arena with a strong 2v2 recommendation and a secret Mega Health rhythm to spice up control.

## Where It Came From

headshot credited danimal's guns-and-ammo texture idea and recommended 2-on-2 play in the original readme. Grind is from the era when authors were still finding the Quake II duel language in public.

## How It Plays

Works well as a bridge between duel and small-team play. Combat Armor appears three times in the register, and the Mega Health can swing a fight if players remember the route.

## Item Register

Counts are taken from the final Muff Mode BSP entity data.

| Category | Register |
| --- | --- |
| Spawns | Deathmatch spawn x6, Intermission camera x1, Start point x1 |
| Weapons | Chaingun x1, Grenade Launcher x1, HyperBlaster x1, Railgun x1, Rocket Launcher x1, Super Shotgun x2 |
| Ammo | Bullets x4, Bullets (large) x1, Cells x1, Grenades x4, Rockets x2, Shells (large) x3, Slugs x3 |
| Utility | Adrenaline x1, Combat Armor x3, Armor Shard x9, Health x3, Large Health x5, Mega Health x1, Small Health x3, Ammo Pack x1 |

## Preserved Files

| File type | Link |
| --- | --- |
| Remaster source map | [mm-grind.map](source-maps/mm-grind.map) |
| Original readme | [grind-readme.txt](original-readmes/grind-readme.txt) |
| Original BSP | [grind.bsp](original-bsps/grind.bsp) |

## Sources

- Final Muff Mode BSP/source data: [source-maps/mm-grind.map](source-maps/mm-grind.map).
- Preserved original readme: [original-readmes/grind-readme.txt](original-readmes/grind-readme.txt).
- Preserved original BSP: [original-bsps/grind.bsp](original-bsps/grind.bsp).
- DondeQ2 map archive: [https://dondeq2.com/2017/10/24/webman-twists-map-collection/](https://dondeq2.com/2017/10/24/webman-twists-map-collection/).
