# Railgun 101

[Map Guide](index.md) | [Gameplay Reference](../gameplay-reference.md)

| Field | Details |
| --- | --- |
| Filename | `mm-rail101` |
| Development status | Final release |
| Original map | 2box4 / Railgun 101 |
| Original author | Thai "SwanSong" Pham |
| Original release date | 23 May 1998 |
| Recommended players | 2-6 |
| Good fits | Instagib, rail practice, aim warmups |
| Original readme | [2box4-readme.txt](original-readmes/2box4-readme.txt) |
| Original BSP | [2box4.bsp](original-bsps/2box4.bsp) |
| Remaster source map | [mm-rail101.map](source-maps/mm-rail101.map) |

## Why Play It

A stripped-down rail practice space: boxes, slugs, sightlines, and almost nothing to distract from aim.

## Where It Came From

SwanSong's original readme describes Railgun 101 as a small map for practicing railgun skills. It became a familiar warmup and Instagib-style room in Quake II server culture.

## How It Plays

Use it for Instagib or warmups. The entity register intentionally has no weapon pickups, only slugs and a single armor shard, so the ruleset or server setup should supply the intended weapon behavior.

## Item Register

Counts are taken from the final Muff Mode BSP entity data.

| Category | Register |
| --- | --- |
| Spawns | Deathmatch spawn x6, Intermission camera x1, Start point x1 |
| Weapons | No weapon pickups |
| Ammo | Slugs x4 |
| Utility | Armor Shard x1 |

## Preserved Files

| File type | Link |
| --- | --- |
| Remaster source map | [mm-rail101.map](source-maps/mm-rail101.map) |
| Original readme | [2box4-readme.txt](original-readmes/2box4-readme.txt) |
| Original BSP | [2box4.bsp](original-bsps/2box4.bsp) |

## Sources

- Final Muff Mode BSP/source data: [source-maps/mm-rail101.map](source-maps/mm-rail101.map).
- Preserved original readme: [original-readmes/2box4-readme.txt](original-readmes/2box4-readme.txt).
- Preserved original BSP: [original-bsps/2box4.bsp](original-bsps/2box4.bsp).
- DondeQ2 map archive: [https://dondeq2.com/2017/10/24/webman-twists-map-collection/](https://dondeq2.com/2017/10/24/webman-twists-map-collection/).
