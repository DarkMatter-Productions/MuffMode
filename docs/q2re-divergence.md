...
- The **Protection powerup is preserved**: `src/g_combat.cpp:698`
  `take = RS(RS_Q2RE) ? 0 : ceil(take/2)` → under ruleset 1, `take = 0`, identical to q2re's
  `invincible_time` path (`q2re-src/g_combat.cpp:672`).
- **Monster AI is vanilla**: `src/g_ai_new.cpp` is just stock q2re "blocked" navigation
...
Verified in `src/g_main.cpp` vs `q2re-src/g_main.cpp`.

### Changed defaults on cvars that exist in both

| Cvar | q2re | MuffMode | Effect |
|------|------|----------|--------|
| `deathmatch` | `0` | `1` (`g_main.cpp:402`) | Fallback mode is DM. Overridden by engine/menu for campaign. MuffMode is a DM-first mod. |
| `skill` | `1` | `3` (`g_main.cpp:532`) | Fallback skill is Hard. Overridden by menu skill selection. |
| `coop` | `0` | `0` | Unchanged. |
| `g_dm_force_respawn` | `0` | `1` (`g_main.cpp:594`) | Forced respawn after death (real DM behavior change). |
| `g_map_list_shuffle` | `0` | `1` (`g_main.cpp:632`) | Map rotation is shuffled. |
...
| Cvar | Default | Effect |
|------|---------|--------|
| `g_dm_do_warmup` | `1` (`g_main.cpp:591`) | Warmup phase before the match starts. |
| `g_dm_force_respawn_time` | `3` (`g_main.cpp:595`) | 3-second forced respawn delay. |
| `g_dm_respawn_delay_min` | `1` (`g_main.cpp:608`) | Minimum 1s before a player may respawn. |
| `g_dm_holdable_adrenaline` | `1` (`g_main.cpp:596`) | Adrenaline becomes a holdable item. |
| `g_dm_powerup_drop` | `1` (`g_main.cpp:606`) | Players drop powerups on death. |
| `g_dm_crosshair_id` | `1` (`g_main.cpp:589`) | Crosshair shows teammate/target ID. |
| `g_dm_spawnpads` | `1` (`g_main.cpp:613`) | Spawn pad effects. |
| `g_dm_respawn_point_min_dist` | `256` (`g_main.cpp:609`) | Minimum distance between reused spawns. |
...
MuffMode it **also grants +30% movement speed** (`MaxSpeed()`, `src/p_move.cpp`,
...
(`src/muffmode/mm_ruleset.cpp`), applied where the synced `ps.pmove.haste` flag is set
(`src/p_client.cpp:4023`). Under `RS_Q2RE`/`RS_VANILLA_PLUS` the DualFire powerup now matches stock
...
- Step height uses `STEPSIZE_BELOW = 20` instead of `STEPSIZE = 18` (`src/game.h:50`; used at
  `p_move.cpp:431, 490, 1091`).
- Jump gets `jump_height += 4` (`p_move.cpp:1004-1007`).
...
`pml.origin` for upper sample points (`p_move.cpp:861`). Minor correctness change.

### Entity physics (`g_phys.cpp`)
...
- **Weapons** — `src/muffmode/mm_ruleset_weapons.cpp`: machinegun, chaingun, rocket launcher,
  hyperblaster, railgun (dmg + kick), BFG, plasma beam, shotgun (dmg + pellets), slug pickup — all
  default to q2re values.
- **Items** — `src/muffmode/mm_items_rules.cpp`: armor tiers, health pickup caps/amounts, health
...
- **Combat** — `src/g_combat.cpp`: armor protection percentages (`277/279`, Q3A-only branch),
  Protection powerup (`698`), rocket splash knockback (`g_weapon.cpp:794`, MM-only branch).
- **DualFire/haste movement buff** — `MM_RulesetHasteBoostsMovement()` (`src/muffmode/mm_ruleset.cpp`)
  suppresses the +30% movespeed under `RS_Q2RE`/`RS_VANILLA_PLUS` (fire-rate-only DualFire, like
  q2re); other rulesets keep it.
- **Movement/world** — fall-damage thresholds (`mm_ruleset.cpp`), landing footstep event
  (`g_phys.cpp:897`, Q1-only branch), plasma beam range (`g_weapon.cpp:1346`, MM/VP/QC-only).
- **Spawn** — `src/muffmode/mm_spawn_filter.cpp` / `mm_spawn_loadout.cpp`: no weapon remapping
...
- **Flag scoring constants** — `src/g_ctf.cpp:7-27` matches `q2re-src/ctf/g_ctf.h:48-73`: capture
...
- **Techs** — renamed but identical magnitudes under `RS_Q2RE`:
  Disruptor Shield = Resistance `dmg/2` (`g_items.cpp:1055`),
  Power Amp = Strength `dmg*2` (`g_items.cpp:1069`),
  Time Accel = Haste, **weapon-speed only** (`g_items.cpp:1096`) — note this CTF tech does **not**
  grant movement speed; that buff belongs to the separate DualFire powerup (§4),
  AutoDoc = Regeneration `+5/500ms → 150` (`mm_items_rules.cpp:283`, the `1_sec` slowdown is
  `RS_MM`-only).
- **Grapple** — same defaults: fly 650 / pull 650 / damage 10 (`src/g_main.cpp:507-509` vs
  `q2re-src/g_main.cpp:256-258`), same FLY→PULL→HANG states.

### Divergences
- **Offhand grapple** — new cvar `g_grapple_offhand` (`src/g_main.cpp:506`), **default `0`** (off);
