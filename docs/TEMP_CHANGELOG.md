...
- **`gametype` shows the short name, not the index:** The current-gametype line now reads e.g. `Team Deathmatch (tdm)` instead of `(3)`, matching the name you pass to `gametype <name>`.
- **`maplist` shows both the rotation list and the votable pool:** It previously printed only `g_map_list` when set (hiding `g_map_pool`). Now it lists the rotation maps and the votable pool separately, so players can see every map available on the server.
