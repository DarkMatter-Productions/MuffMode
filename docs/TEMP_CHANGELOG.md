...
- **Arena elimination corpses (CA / CaptureStrike):** Eliminated players keep the brief death pose, then auto-enter chase cam on a teammate. Client corpses during that beat and bodyque leftovers are non-damageable and non-solid so splash/rockets no longer interact with dead bodies. Fixed `T_Damage` uninitialized `asave`/`psave` on arena self-damage (Debug RTC #3 crash after death).
