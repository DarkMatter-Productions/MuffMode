...
MuffMode stores each server-side version-2 player profile in `baseq2/pcfg/profiles/sid-<encoded-social-id>.json`, using a safe hex encoding of the engine-provided social ID and a directory that cannot collide with WORR's legacy filename scheme. Missing or unusually long social IDs keep session-only state instead of using fallback filenames. The versioned JSON schema records current, original, and up to 16 previous player-name aliases; first-seen, last-seen, and last-updated timestamps; display, audio, follow, and skin preferences; a custom weapon preference order; per-gametype skill ratings, latest rating changes, and match counts; and aggregate match totals for wins, losses, draws, abandons, play time, and best rating. Profile data never grants administrator or ban authority.

A new player's first 20 rated matches in a gametype use a higher Elo K-factor (48 instead of the usual 32), so their rating converges toward their true skill faster instead of taking dozens of matches to climb out of the 1500 default. This is tracked per gametype, so a veteran duelist starting Capture the Flag for the first time is still provisional there. Once a player passes 20 matches in a gametype, further rating changes there use the standard K-factor.
...
Arena Rooms run multiple independent room series at once, so they remain unranked and are not exported; their live Player Stats menu continues to use room-local counters. Horde is co-op PvE rather than a contest between players, so it is unranked for the same reason — no Elo settlement, no awards reel, and match totals are not persisted to player profiles. For the same reason, MuffMode's `MATCH_LOGGING_STATUS_API_V1` smoke status validates the singleton match schema and its real atomic catalog write path, but does not advertise WORR tournament-series fields.
