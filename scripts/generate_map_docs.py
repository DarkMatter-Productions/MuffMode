...
def status_for(stem: str, final: bool = False) -> str:
    ...
    match = re.search(r"-build(\d*)$", stem)
    if match:
        return f"Work build {match.group(1)}".rstrip()
    return "In development"


def status_rank(stem: str) -> tuple[int, int]:
    for pattern, rank in [(r"-rc(\d+)$", 4), (r"-b(\d+)$", 3), (r"-a(\d+)$", 2), (r"-build(\d*)$", 1)]:
...
def sources_list(map_info: dict[str, object]) -> str:
    ...
    for label, url in map_info.get("source_urls", []):
...
def select_dev_entries(dev_src: Path, dev_bsps: Path) -> list[dict[str, object]]:
    ...
    for record in stem_records.values():
        title = record["title"] or re.sub(r"-(rc|b|a|build)\d*$", "", str(record["stem"])).replace("mm-", "").replace("-", " ").title()
        record["title"] = title
...
def write_asset_readmes(docs: Path, source_rows: list[tuple[str, str, str]], dev_selected: list[dict[str, object]]) -> None:
    ...
    lines = [
        ...
        "",
        "This folder mirrors only selected `.map` files from `MuffMode-Map-Remasters/dev`, one newest apparent state per map identity. It does not contain BSP files.",
        "",
